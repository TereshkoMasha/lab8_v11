...
class IndexOne2One implements Serializable, IndexBase {
    ...
    public void put( String key, long value ) { map.put(key, new Long( value )); }
...
public class Index implements Serializable, Closeable {
    ...
    IndexOne2One departNumber;
    IndexOne2One   fullname;
    IndexOne2N   personnelNumber;

    public void test( Employee employee ) throws KeyNotUniqueException {
        assert( employee != null );
        if ( departNumber.contains(employee.departmentNumber)) {
            throw new KeyNotUniqueException( employee.departmentNumber );
        }
        if ( fullname.contains( employee.fullName )) {
            throw new KeyNotUniqueException( employee.fullName );
        }
    }

    public void put( Employee employee, long value ) throws KeyNotUniqueException {
        test( employee );
        departNumber.put( employee.departmentNumber, value );
        fullname.put( employee.fullName, value);
        personnelNumber.put(employee.personnelNumber,value);
    }


    public Index()  {
        departNumber   = new IndexOne2One();
        fullname = new IndexOne2One();
        personnelNumber = new IndexOne2N();
    }
